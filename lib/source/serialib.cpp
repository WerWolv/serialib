...
SerialDevice::SerialDevice(serialib::SerialDevice &&other) noexcept {
...
SerialDevice &SerialDevice::operator=(serialib::SerialDevice &&other) noexcept {
    currentStateDTR = other.currentStateDTR;
    currentStateRTS = other.currentStateRTS;
...
/*!
  ...
  */
char SerialDevice::openDevice(const char *portName, unsigned int bauds,
...
#if defined (_WIN32) || defined( _WIN64)
    // Open serial port
    hSerial = CreateFileA(portName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
bool SerialDevice::isDeviceOpen()
{
#if defined (_WIN32) || defined( _WIN64)
...
/*!
  ...
  */
int SerialDevice::writeChar(char byte)
{
#if defined (_WIN32) || defined( _WIN64)
    ...
    if(!WriteFile(hSerial, &byte, 1, &dwBytesWritten, nullptr)) return -1;
...
/*!
     ...
     \return    -1 error while writting data
  */
int SerialDevice::writeString(const char *receivedString)
{
#if defined (_WIN32) || defined( _WIN64)
    // Number of bytes written
    DWORD dwBytesWritten;
    // Write the string
    if(!WriteFile(hSerial, receivedString, strlen(receivedString), &dwBytesWritten, nullptr))
...
/*!
     ...
     \return -1 error while writting data
  */
int SerialDevice::writeBytes(const void *buffer, unsigned int nbBytes)
{
#if defined (_WIN32) || defined( _WIN64)
    // Number of bytes written
    DWORD dwBytesWritten;
    // Write data
    if(!WriteFile(hSerial, buffer, nbBytes, &dwBytesWritten, nullptr))
        // Error while writing, return -1
        return -1;
    // Write operation successful
    return 1;
...
/*!
  ...
  */
int SerialDevice::readChar(char *pByte,unsigned int timeOut_ms)
{
#if defined (_WIN32) || defined(_WIN64)
    ...
    if(!ReadFile(hSerial, pByte, 1, &dwBytesRead, nullptr)) return -2;
...
/*!
  ...
  */
int SerialDevice::readStringNoTimeOut(char *receivedString, char finalChar, unsigned int maxNbBytes)
{
...
/*!
     ...
     \return -3 MaxNbBytes is reached
  */
int SerialDevice::readString(char *receivedString,char finalChar,unsigned int maxNbBytes,unsigned int timeOut_ms)
{
...
/*!
     ...
     \return -2 error while reading the byte
  */
int SerialDevice::readBytes (void *buffer,unsigned int maxNbBytes,unsigned int timeOut_ms, unsigned int sleepDuration_us)
{
#if defined (_WIN32) || defined(_WIN64)
    // Avoid warning while compiling
    (void)(sleepDuration_us);
    ...
    if(!ReadFile(hSerial, buffer, (DWORD)maxNbBytes, &dwBytesRead, nullptr))  return -2;
...
*/
char SerialDevice::flushReceiver()
{
#if defined (_WIN32) || defined(_WIN64)
    // Purge receiver
...
*/
bool SerialDevice::DTR(bool status)
{
    if (status)
        // Set DTR
        return this->setDTR();
...
*/
bool SerialDevice::RTS(bool status)
{
    if (status)
        // Set RTS
        return this->setRTS();
